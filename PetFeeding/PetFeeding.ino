...
void setup(void)
{
  ...
  //Setup Load Cell Sensor
  scale.begin(A1, A0);
  scale.set_scale(calibrationLc);
  scale.tare();

  Serial.println(F("Elechouse Voice Recognition V3 Module \"train\" sample."));

  myservo.attach(4);//Setup pin Servo Motor
...
//  Serial.println(GRAM);
  ...
  if (bufferVr == true && GRAM < 190) {
    Serial.println("Open feed");
    digitalWrite(6, HIGH);
    myservo.write(90);
  }
  if (GRAM >= 190) {
    Serial.println("Close feed");
    digitalWrite(6, LOW);
    digitalWrite(7, HIGH);
    digitalWrite(5, LOW);
    myservo.write(0);
